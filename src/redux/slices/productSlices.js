...
export const fetchProducts = createAsyncThunk("products/fetchProducts", async (filterParams = {}, { rejectWithValue }) => {
    try {
        ...
        if (filterParams.keyword?.trim()) {
            params.keyword = filterParams.keyword.trim();
        }

        // Include category if it's a non-empty array
        if (Array.isArray(filterParams.category) && filterParams.category.length > 0) {
            params.category = filterParams.category.join(",");
        }

        // Include color if it's a non-empty array
        if (Array.isArray(filterParams.color) && filterParams.color.length > 0) {
            params.color = filterParams.color.join(",");
        }


        // Include priceRange if it differs from the default [0, 1000]
        if (Array.isArray(filterParams.priceRange) &&
            (filterParams.priceRange[0] !== 0 || filterParams.priceRange[1] !== 1000)
        ) {
        ...
        // Include sort if it's not an empty string
        if (filterParams.sort?.trim()) {
            params.sort = filterParams.sort.trim();
        }  
