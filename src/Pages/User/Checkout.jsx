...
init("OxJ-ujUPZE2bAuJdk"); // ← your EmailJS public key
...
const Checkout = () => {
  ...
  const shippingCost = 0;      // adjust if you have real shipping logic
  ...
  const faqData = [
    {
      ...
      answer: "We accept returns within 30 days if the product is unused and in its original condition.",
    },
    {
    ...
    },
    {
      ...
      answer: "Yes, shipping charges and delivery times vary by destination.",
    },
  ];
  ...
  const handleOrderCreation = async (paymentResult) => {
    ...
    const missing = Object.entries(shippingDetails).find(([, v]) => !v.trim());
    if (missing) {
    ...
    try {
      ...
      await emailjs.send(
        "service_jbo1u2h",            // ← your Service ID
        "template_qnsdz0r", // ← your Template ID
        templateParams
      );
